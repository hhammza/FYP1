...
class KmerResistancePredictor:
    def __init__(self, model_dir=None):
        self.model_dir = str(model_dir) if model_dir else default_model_dir()
        self.model = None
        self.scaler = None
        self.ab_list = []
    ...
    def _load(self):
        ...
        if os.path.exists(model_path):
            try:
                ...
                _log.info(f"Model loaded. Antibiotics: {len(self.ab_list)}")
            except Exception as e:
                _log.warning(f"Could not load model: {e}")
        else:
            _log.info("No trained model. Using heuristic predictions.")
    ...
    def predict(self, fasta_text, antibiotic, threshold=0.5):
        ...
        if not self.is_trained:
            ...
            try:
                ...
                if self.scaler:
                    # The scaler was fitted on the k-mer block alone (see
                    # train_models.py), so only those columns may be scaled.
                    # Passing the full vector raises a feature-count mismatch.
                    feat[:N_KMERS] = self.scaler.transform(
                        feat[:N_KMERS].reshape(1, -1)
                    )[0]
                prob = float(self.model.predict_proba(feat.reshape(1, -1))[0][1])
            except Exception as e:
                _log.warning(f"Prediction error: {e}")
                prob = self._heuristic_predict(sequence, antibiotic)
