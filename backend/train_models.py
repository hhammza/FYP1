...
# Antibiotic names, drug classes and phenotype labels, shared with
# experiments/lib/data_prep.py and the predictors
from amr_constants import (ANTIBIOTIC_ALIASES, DRUG_CLASS_MAP,  # noqa: E402,F401
                           KMER_TRAIN_PHENOTYPES, LGBM_TRAIN_PHENOTYPES, PHENOTYPE_MAP)
...
def clean_amr_data(df_raw):
    ...
    merge_map = {p: ('Resistant' if PHENOTYPE_MAP[p] else 'Susceptible') for p in LGBM_TRAIN_PHENOTYPES}
...
def train_kmer(data_dir, model_dir, max_files=None):
    ...
    LABEL_MAP = {p: PHENOTYPE_MAP[p] for p in KMER_TRAIN_PHENOTYPES}
