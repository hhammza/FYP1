...
**Files I own:** `frontend/`, `backend/api/`, `backend/backend/settings.py`, backend and frontend tests, Dockerfile and Railway config.
...
- [ ] **`metrics.json` format** agreed with Hamza; get a sample file to build against
- [ ] **Genome prediction response** agreed with Hamza: today's response plus `genes_found: [{gene, drug_class}]`
- [ ] **Timeline + RL response** agreed with Ali: weekly susceptible, intermediate and resistant fractions, plus a `policy` list
- [ ] **Batch CSV template and response** defined by me: columns `antibiotic, genus, species, taxon_id, mic_value, mic_sign`; one result row per input row with an `error` column
...
- [ ] `frontend/app.py:ANTIBIOTICS`: replace `rifampin` with `rifampicin`
- [ ] Consider adding common drugs the list lacks (1,000+ rows each): spectinomycin, ceftiofur, ampicillin/sulbactam, sulfisoxazole, pefloxacin, penicillin, ceftazidime/avibactam, ceftolozane/tazobactam, cefixime, telithromycin, moxifloxacin, clarithromycin, temocillin, cefpirome, florfenicol
