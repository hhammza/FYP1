...
| From Ali | **Cleaning v5** (`77bc855`): `Genome ID` is now read as text in `data_prep.py` and `train_models.py`. As a number it merged 3,312 genomes into others and dropped 36,850 labelled rows (2.4%); v5 has 1,558,494 rows and 131,385 genomes. **To do (Hamza):** retrain and promote the forecaster on v5, rerun `evaluate_shipped.py`, re-run the registry configs you quote, and read `genome_id` with `dtype=str` in `promote.py:150` (its test-genome count merges IDs the same way) | Week 2 | [ ] |
| From Ali | `Data/mapped_output/` Genome IDs fixed (`b05d718`): 68 genomes had lost a trailing zero (e.g. `1055537.10` → `1055537.1`; `1038927.40` had merged with `1038927.4`); 888 misattributed rows of 20 genomes corrected later in `77bc855`. **To do (Hamza):** rerun `experiments/evaluate_shipped.py` so `kmer_metrics.json` uses the fixed IDs | Week 2 | [ ] |
